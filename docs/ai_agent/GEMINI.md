...
Todos os novos prompts de agente de IA devem ser complementares a este documento. As diretrizes gerais definidas aqui em `@/docs/ai_agent/GEMINI.md` são herdadas por todos os outros agentes e **não devem ser repetidas**.
...
4.  **Manipulação de Caminhos de Arquivo (File Paths)**:
    - **Compatibilidade Multi-SO**: O código que manipula caminhos de arquivo deve ser compatível com diferentes sistemas operacionais. Sempre que um caminho for recebido como entrada (ex: de um arquivo de configuração ou UI), ele deve ser tratado de forma agnóstica.
    - **Salvamento em Rede**: A lógica de salvamento de arquivos deve ser capaz de interpretar e utilizar corretamente caminhos de rede, tanto no formato Windows (ex: `C:\Pasta\Subpasta` ou `\\servidor\share\pasta`) quanto no formato Linux/WSL (ex: `/mnt/c/Pasta/Subpasta` ou `/servidor/share/pasta`). Utilize bibliotecas como `pathlib` em Python para manipular caminhos de forma segura e compatível.
    - **Diretórios de UI (Ex: Gradio)**: Componentes de UI que renderizam, exibem ou manipulam arquivos devem ter permissão explícita para acessar diretórios fora da pasta raiz do projeto. É **obrigatório** configurar propriedades de segurança, como `allowed_paths` ou equivalentes, na inicialização da interface. Em ambientes WSL, a abordagem mais robusta é incluir a raiz de montagem `/mnt` e, se necessário, a raiz do sistema (`/`) para garantir a compatibilidade com qualquer caminho de rede mapeado.
